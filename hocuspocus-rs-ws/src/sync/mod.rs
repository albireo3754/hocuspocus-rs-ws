...
pub trait Protocol {
    ...
    fn handle_sync_step1(
        ...
        debug!("pray - handle_sync_step1 - update: {:?}", update);
        Ok(Some(Message::Sync(SyncMessage::SyncStep2(update))))
    }

    fn sync_step1(&self, awareness: &Awareness) -> Result<Message, Error> {
        let sv = awareness.doc().transact().state_vector();
        Ok(Message::Sync(SyncMessage::SyncStep1(sv)))
    }

    fn init_awareness(&self, awareness: &Awareness) -> Result<Message, Error> {
        let update = awareness.update()?;
        Ok(Message::Awareness(update))
    }

    fn awareness(&self, awareness: &Awareness) -> Result<Message, Error> {
        let update = awareness.update()?;
        Ok(Message::Awareness(update))
    }
    ...
    fn handle_sync_step2(
        ...
        Ok(Some(Message::SyncStatus(true)))
    }
    ...
    fn handle_auth_success(&self, _awareness: &Awareness, read_write: bool) -> Message {
        Message::Auth(
            if read_write {
...
impl Encode for Message {
    fn encode<E: Encoder>(&self, encoder: &mut E) {
        match self {
            ...
            Message::Auth(reason, authenticated) => {
                encoder.write_var(MSG_AUTH);
                if *authenticated {
                    encoder.write_var(AUTHENTICATED);
                } else {
...
pub enum Error {
    ...
    /// Generic error, usually used for custom protocol handling.
    #[error("{0}")]
...
mod test {
    ...
    fn message_encoding() {
        ...
        let messages = [
            ...
            Message::Auth(Some("reason".to_string()), false),
            Message::AwarenessQuery,
        ];
